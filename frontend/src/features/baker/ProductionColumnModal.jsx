  ...
  return (
    <Modal onClose={onClose} maxWidthClass="max-w-xl">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <span className="text-caption font-medium uppercase tracking-wide text-stone-500">Партия на дату</span>
      ...
      <div className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2">
        <div>
          <strong className="block text-body text-stone-900">Заказов: {orders.length}</strong>
          <span className="text-note text-stone-600">
        ...
        {available.length > 0 && (
          <button
            type="button"
            className="shrink-0 text-note font-semibold text-stone-700 underline decoration-stone-300 underline-offset-2"
        ...
        {orders.map((order) => {
          ...
          if (worked) {
            return (
              ...
              >
                <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-stone-700 text-caption font-bold text-white" aria-hidden="true">✓</span>
                <span className="min-w-0 flex-1">
                  <strong className="block truncate text-body text-stone-950">{order.number}</strong>
                  <span className="block truncate text-note text-stone-600">{orderSource(order)} · {order.items?.length || 0} поз.</span>
                </span>

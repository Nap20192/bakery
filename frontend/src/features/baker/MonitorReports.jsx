...
export function MonitorReports({ monitor, onCalculate, loading, canCalculate = true }) {
    ...
    if (onCalculate) {
      return (
        <div className="mt-3 flex flex-col items-center gap-2 rounded-lg border border-dashed border-stone-300 bg-stone-50 px-4 py-8 text-center">
          <p className="m-0 text-body text-stone-500">Расчёт расхода теста по заказам.</p>
  ...
  if (monitor.total_reports?.length) {
    // Основной вид — только итоги партии; раскладка «по заказам» живёт в
    // расшифровке, чтобы не раздувать экран.
    return (
  ...
  return (
    <Modal onClose={onClose}>
      <div className="mb-3 flex items-center justify-between gap-2">
        <h3 className="m-0 text-title font-semibold leading-6 text-stone-950">Расшифровка расчёта</h3>
...
function DetailsSection({ title = '', reports }) {
  ...
  return (
    <section className="space-y-2">
      {title && <h4 className="m-0 text-note font-semibold uppercase tracking-wide text-stone-500">{title}</h4>}
      {used.map(({ code, report }) => (
        <article className="overflow-hidden rounded-lg border border-stone-200" key={code}>
          <header className="flex items-baseline justify-between gap-3 border-b border-stone-200 bg-stone-50 px-3 py-2">
            <strong className="min-w-0 break-words text-body font-semibold leading-5 text-stone-950">{report.ingredient.product_name}</strong>
            <span className="shrink-0 text-input font-bold leading-5 tabular-nums text-stone-950">
              {formatQuantity(report.ingredient.quantity)} <span className="text-note font-medium text-stone-500">{report.ingredient.unit}</span>
            </span>
          </header>
          {report.breakdown?.length > 0 && (
            <div>
              <p className="m-0 bg-stone-50/60 px-3 py-1 text-caption font-medium uppercase tracking-wide text-stone-400">По блюдам</p>
              <ul className="m-0 list-none divide-y divide-stone-100 p-0">
                {report.breakdown.map((item) => (
                  <li className="flex items-baseline justify-between gap-3 px-3 py-1 text-body" key={`${code}-${item.order_item_code}-${item.order_item_name}`}>
                    ...
                    <span className="shrink-0 leading-5 tabular-nums text-stone-900">
                      {formatQuantity(item.ingredient_quantity)} <span className="text-note text-stone-500">{report.ingredient.unit}</span>
                    </span>
                  </li>
                ))}
              </ul>
          ...
          {report.components?.length > 0 && (
            <div className="border-t border-stone-200">
              <p className="m-0 bg-stone-50/60 px-3 py-1 text-caption font-medium uppercase tracking-wide text-stone-400">Состав</p>
              <ul className="m-0 list-none divide-y divide-stone-100 p-0">
                {report.components.map((item, index) => (
                  <li className="flex items-baseline justify-between gap-3 px-3 py-1 text-body" key={`${code}-c-${item.product_code || index}`}>
                    <span className="min-w-0 break-words leading-5 text-stone-600">{item.product_name}</span>
                    <span className="shrink-0 leading-5 tabular-nums text-stone-900">
                      {formatQuantity(item.quantity)} <span className="text-note text-stone-500">{item.unit}</span>
                    </span>
                  </li>
                ))}
              </ul>
...
function ReportSection({ title = '', reports }) {
  ...
  return (
    <section className="space-y-2">
      {title && <h3 className="m-0 px-0.5 text-caption font-medium uppercase tracking-wide text-stone-500">{title}</h3>}
      {used.map(({ code, report }) => (
      ...
      {unused.length > 0 && used.length > 0 && (
        <p className="m-0 px-0.5 text-note leading-5 text-stone-400">
...
function DoughCard({ report }) {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
      <header className="flex items-baseline justify-between gap-3 border-b border-stone-200 bg-stone-50 px-3 py-2">
        <strong className="min-w-0 break-words text-body font-semibold leading-5 text-stone-950">{ing.product_name}</strong>
        <span className="shrink-0 text-title font-bold leading-5 tabular-nums text-stone-950">
          {formatQuantity(ing.quantity)} <span className="text-note font-medium text-stone-500">{ing.unit}</span>
        </span>
      </header>
      {report.components?.length > 0 && (
        <ul className="m-0 list-none divide-y divide-stone-100 p-0">
          {report.components.map((item, index) => (
            <li
              ...
              className="flex items-baseline justify-between gap-3 px-3 py-1.5 text-body"
            >
              <span className="min-w-0 break-words leading-5 text-stone-600">{item.product_name}</span>
              <span className="shrink-0 leading-5 tabular-nums text-stone-900">
                {formatQuantity(item.quantity)} <span className="text-note text-stone-500">{item.unit}</span>
